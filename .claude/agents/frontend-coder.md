...
3. 80점 미만이면 감점 사유 중 **감점이 큰 항목부터** CSS/HTML을 **1회 수정**하고 스크립트를 다시 실행해 재채점하세요. (무한 반복 금지 — 최대 1회 수정 사이클로 시간/비용 통제. 재수정 후에도 80점 미만이면 억지로 통과 처리하지 말고 README에 **실제 점수와 감점 사유를 숨김없이** 기록한 뒤 다음 단계로 진행 — main-orchestrator가 이를 보고 재작업 여부를 판단합니다.)
...
- [ ] **시각 검수 점수 80점(B) 이상**, 또는 미만이면 그 사실과 감점 사유를 README에 숨김없이 기록함

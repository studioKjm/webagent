...
```json
{
  ...
  "footer": {
    ...
    "copyright": "© {현재 연도} YourCompany. All rights reserved."
  }
}
```

`{현재 연도}`는 생성 시점의 실제 연도로 치환하세요(하드코딩된 연도를 그대로 복사하지 말 것).

---

## 마이크로카피 가이드 — 선택 아님, 필수

헤드라인·CTA 같은 "큰 카피" 외에, 사용자가 실제로 상호작용하는 짧은 순간의 문구도 함께 작성하세요. 없으면 `frontend-coder`가 기본 영문/무미건조한 문구를 그대로 쓰게 됩니다. **`layout-designer`가 레이아웃 JSON에 `"states"` 필드(idle/loading/error/success 등)를 명시한 섹션은, 해피패스 카피보다 먼저 이 상태별 카피부터 작성하세요** — "정상 작동할 때"의 카피는 쉽지만, 실패·대기 상태의 카피가 빠지면 실제 사용 시 어색한 기본 문구가 그대로 노출됩니다.

### 상태별 마이크로카피

| 상황 | 나쁜 예 | 좋은 예 |
|-----|--------|--------|
| 폼 제출 로딩 | "Loading..." | "제출하는 중..." / "거의 다 됐어요" |
| 제출 성공 | "Success" | "완료됐어요! 확인 이메일을 보냈어요" |
| 빈 상태(결과 없음) | "No data" | "아직 항목이 없어요 — 첫 번째를 추가해보세요" |
| 폼 필드 에러 | "Invalid input" | "이메일 형식을 확인해주세요 (예: name@company.com)" |
| 네트워크 오류 | "Error occurred" | "연결이 원활하지 않아요. 다시 시도해주세요" |
| 저장 중 | "Saving" | "저장 중..." → "저장됨 ✓" (완료 후 짧게 표시) |

### 개인화 카피 (선택)

레퍼런스 URL 분석이나 사용자 입력에 특정 세그먼트가 명확한 경우(예: "개발자 타겟"), 헤드라인/CTA에 그 맥락을 반영한 변형을 하나 더 제안할 수 있습니다.
```
기본: "무료로 시작하기"
개발자 맥락: "npm install로 시작하기" 같은 타겟 특화 표현 고려
```
단, 과도한 개인화는 범용성을 해치므로 명확한 근거가 있을 때만 적용하세요.
...
- [ ] JSON 형식으로 출력
- [ ] 맞춤법 검사 완료
- [ ] 주요 상태(로딩/성공/에러/빈 상태)에 마이크로카피 제공

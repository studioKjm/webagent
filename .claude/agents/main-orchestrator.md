...
Task 도구로 frontend-coder를 **3번, 한 메시지 안에서 동시에** 호출하세요(3개 버전은 서로 독립적이라 병렬 실행이 안전하며, 순차 호출 대비 소요 시간이 크게 줄어듭니다):
...
```
→ 이 블록을 Version 1/2/3용으로 각각 채워 **하나의 응답에서 3개 Task 호출을 동시에** 보내세요. 3번 모두 **서로 다른 레이아웃 + 서로 다른 디자인 토큰**을 사용합니다. 동일 레이아웃에 색상만 바꿔 재사용하지 마세요.

**예외**: 로컬 리소스(디스크 I/O, 동일 파일 동시 쓰기 등) 경합으로 병렬 실행이 실패하거나 불안정하면, 그때만 순차 호출(Step 4→5→6)로 되돌아가세요.

**각 호출이 완료됐다고 보고해도 그대로 믿지 마세요.** frontend-coder의 텍스트 보고만으로 판단하지 말고, **Grep 도구로 해당 버전의 `README.md`에서 `시각 검수 점수` 문자열을 직접 검색**해 확인하세요:

```
Grep pattern: "시각 검수 점수" 경로: output/{project}/version-{n}/README.md
```

- **매치가 없으면** frontend-coder가 "6.5 시각 검수"를 건너뛴 것으로 간주하고, 시각 검수를 실행해서 README에 정해진 형식(`**시각 검수 점수**: N/100 (등급) — 수정 사이클 n회`)으로 점수를 기록하라고 다시 지시하세요. 재지시 없이 넘어가지 마세요.
- **매치가 있으면** 점수와 `수정 사이클 n회`의 n을 함께 읽으세요.

**80점(B) 미만인 버전은 그대로 사용자에게 완료로 보고하지 마세요.** frontend-coder가 최대 3회 수정 사이클을 다 써도 80점 미만이면, 사용자에게 보고할 요약에 그 버전의 점수와 감점 사유를 숨김없이 포함하고 "이 버전은 기준 미달이며 추가 수정이 필요하다"고 명시하세요. 점수를 임의로 낙관적으로 재해석하거나 생략하지 마세요.

**⚠️ 수정 사이클 상한 위반 감지 (n > 3)**: frontend-coder에게 "최대 3회"라고 지시해도 실제로 지켜지지 않을 수 있음이 실측으로 확인됐습니다(2026-09-10 실측에서 한 버전이 5회를 돌린 사례). 텍스트 지침만으로 강제할 수는 없으므로, main-orchestrator가 사후에라도 **반드시 감지하고 사용자에게 투명하게 보고**하세요:
- n이 3을 초과하면 요약 보고서에 "이 버전은 상한을 초과해 수정됨(n회) — 시간/비용이 예상보다 더 들었을 수 있음"을 명시하세요.
- 원인을 짐작으로 설명하지 말고(예: "아마 결함이 많아서") 사실만 전달하세요 — README의 감점 사유 기록을 참고해 실제 감점 항목을 근거로 드세요.

**사업영향 태그가 있으면 우선순위를 구분해 보고하세요.** README의 감점 사유에 `blocks-conversion` 태그가 붙은 항목이 있으면(점수가 80점을 넘었더라도) 사용자 요약에 별도로 강조하세요 — 전환을 막는 결함은 점수가 기준을 넘겼다는 이유로 묻히면 안 됩니다. `adds-friction`/`cosmetic-only`는 참고용으로만 나열하고 우선순위를 낮게 표시하세요.
...
│ 톤1+고유레이아웃│ 톤2+고유레이아웃│ 톤3+고유레이아웃│
│ 출력:version-1/│ 출력:version-2/│ 출력:version-3/│
│ **병렬 실행**   │                │                │
└────────────────┴────────────────┴────────────────┘
              ↓
┌─────────────────────────────────────┐

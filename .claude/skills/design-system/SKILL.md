...
## 15. 디자인 토큰 스파인 (Token Spine)

매 프로젝트를 완전히 처음부터 만들면 품질이 프로젝트마다 들쭉날쭉하고, 이전에 검증된 좋은 조합이 누적되지 않습니다. `ui-stylist`는 색상/폰트를 결정하기 **전에** [`token-spine.md`](token-spine.md)를 먼저 읽고, primitive(원시 값) → semantic(역할) → component(적용 방식) 3계층으로 정리된 기존 톤 조합을 참고하세요.

- 톤이 비슷하면 **그대로 복제하지 말고 색상 하나 정도는 바꿔서 변형**해 재사용
- 완전히 새로운 조합을 만들어도 되지만, 만들었고 시각 검수(80점 이상)를 통과했다면 프로젝트 완료 후 `token-spine.md`에 항목을 추가해 다음 프로젝트가 이어받을 수 있게 하세요
- 상세 갱신 규칙은 `token-spine.md` 하단 참고

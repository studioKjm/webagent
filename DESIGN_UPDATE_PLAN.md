...
## 11. Phase 6/7 실측 (2026-09-10, `output/design-update-test-r2/`)

같은 FocusFlow 스펙으로 재생성해 Phase 6/7을 실측했다(1차 시도는 사용자 노트북 절전모드로 stall, 재시도로 성공).

- **점수 게이트**: 정상 작동 확인 — 3버전 README 모두 `**시각 검수 점수**: N/100 (등급) — 수정 사이클 n회` 형식 정확히 존재(V1 94/A/2회, V2 92/A/**5회**, V3 92/A/2회), Grep으로 직접 확인(자체 보고 아님)
- **병렬 실행**: frontend-coder 3버전 호출 구간만 보면 약 19.5분(순차였다면 추정 51분 — 약 2.6배) — 하지만 V2가 재수정 상한(3회)을 어기고 5회를 돌면서 그 구간의 병목이 됐고, 전체 파이프라인 시간은 기존 순차 실행(26분)과 거의 같아짐
- **사용자가 브라우저로 직접 확인**: 3버전(에디토리얼/플레이풀/오가닉) 모두 히어로에 실제 제품 비주얼이 폴드 안에 들어오고, 색상·폰트·레이아웃이 실제로 다름 — "이번엔 확실히 다르다"고 확인
- **새로 발견한 회귀**: round 2의 3버전 CSS 전부 **OKLCH 0회, HEX만 사용**(round 1은 111/32/79회) — Phase 2에서 강제한 OKLCH 원칙이 이번 생성에서는 지켜지지 않음. 텍스트 지침 준수가 매 실행마다 100% 보장되지 않는다는 것을 다시 확인한 사례(재수정 상한 위반과 같은 패턴)

**이 브랜치를 `main`에 fast-forward 머지 완료** (2026-09-10, `8a1de6f..d9c2f0f`).

## ✅ Phase 8 완료 (2026-09-10) — 재수정 사이클 상한 위반 감지
- `main-orchestrator.md`: 점수 Grep 확인 시 `수정 사이클 n회`의 n도 함께 읽어, 3 초과 시 요약 보고서에 사실 그대로("상한 초과, n회") 명시하도록 추가. 강제는 못 해도 최소한 은폐되지 않게 함

## ✅ Phase 9 완료 (2026-09-10) — Unhappy-path, 토큰 스파인, 휴리스틱 태깅
- `layout-designer.md`: 신규 "2.5 Unhappy-path 우선 설계" — 폼/동적 미리보기가 있는 섹션은 idle/loading/empty/error/success를 먼저 설계, JSON에 `"states"` 필드로 명시
- `copywriter.md`: 마이크로카피 가이드를 "선택"에서 "필수"로 격상, `states` 필드를 보고 해피패스보다 먼저 작성하도록 지시
- `frontend-coder.md`: 시각 검수 감점표에 "Unhappy-path 상태 누락"(상태당 -6, 실제로 트리거해서 확인) 추가. 신규 "사업영향 태깅" — 10점 이상 감점 항목에 `blocks-conversion`/`adds-friction`/`cosmetic-only` 태그 부여
- `main-orchestrator.md`: `blocks-conversion` 태그 항목은 점수가 기준을 넘어도 사용자 요약에 별도 강조하도록 지시
- 신규 `.claude/skills/design-system/token-spine.md`: 3계층(primitive→semantic→component) 토큰 라이브러리, 오늘까지 실제 생성된 6개 톤(다크 테크니컬/브루탈 미니멀/벤토맥시멀/에디토리얼/플레이풀/오가닉)의 실측 폰트·색상 값으로 시드
- `design-system` 스킬에 "15. 디자인 토큰 스파인" 섹션 추가, `ui-stylist.md`가 작업 전 스파인을 읽고 완료 후 갱신하도록 지시(Edit 툴 권한 추가)

**의도적으로 보류**: 독립 검수 에이전트 도입(점수 자기채점 문제 — 비용 증가 트레이드오프, 별도 논의 필요), 컴포넌트 프리미티브 도입(철학적 긴장 있어 보류), 비용/속도 근본 최적화(투자 필요, 미착수).

## ✅ Phase 10 완료 (2026-09-10) — 비용/속도 최적화 (품질 영향 없는 것만)

"품질에 지장 없는 선에서"라는 제약 하에 진행. 시각 QA 루프(재수정 사이클, 스크린샷 2뷰포트, 점수제)는 품질의 핵심이라 **손대지 않음**. 대신 순수하게 대기 시간만 있던 구간을 찾아 병렬화:

- `main-orchestrator.md`: **Step 1-3(Layout Designer / UI Stylist / Copywriter)을 순차 → 병렬 실행**으로 변경. 셋은 서로의 출력에 의존하지 않음(Layout은 톤별 패턴만, UI Stylist는 색상/폰트만, Copywriter는 제품 정보만 필요 — 상호 참조 없음)이 확인됨. 실측상 이 구간에만 약 7분이 걸렸으므로, 전체 파이프라인(현재 약 26분) 중 정보 손실 없이 줄일 수 있는 순수 대기 시간이었음
- 톤 매트릭스 선택(2단계) 순서를 Layout Designer 호출보다 먼저로 정정 — 기존 "사용 예시" 텍스트에 Layout Designer를 먼저 부르고 톤을 나중에 선택하는 것처럼 보이는 순서 오류가 있었음(실제 실행 로직과는 무관, 문서상 오탈자성 수정)
- 플로우 다이어그램도 3-박스 병렬 형태로 갱신

**검토했지만 채택 안 한 것(품질 리스크 있다고 판단)**: 모바일 스크린샷을 매 사이클 대신 마지막에만 찍기(초기 사이클에서 모바일 결함을 늦게 발견할 위험), 시각 검수 뷰포트 축소. 이런 것들은 비용은 줄이지만 품질 저하 가능성이 있어 이번엔 보류.

**아직 미착수**: 독립 검수 에이전트, 컴포넌트 프리미티브 도입 — 여전히 별도 논의 필요.

## 13. 다음 액션
Phase 8/9/10 모두 아직 실제 생성으로 검증 전이다(지침만 반영, 실측 대기). `output/design-update-test/`, `output/design-update-test-r2/`는 테스트 산출물이며 git에 포함하지 않고 유지 중.
